    ...
    stages {
        ...
        stage('Deploy to Ansible Server') {
            steps {
                ...
                withCredentials([sshUserPrivateKey(credentialsId: '90e346b5-36e9-4e3f-b300-8d1bfbfa6b2d', keyFileVariable: 'SSH_PRIVATE_KEY')]) {
                    sh '''
                    rsync -e "ssh -o StrictHostKeyChecking=no -i $SSH_PRIVATE_KEY" -av --exclude venv/ ./ ec2-user@ec2-44-201-198-53.compute-1.amazonaws.com:/home/ec2-user/ansible-data/
                    '''
                }
            }
        }
        ...
        stage('Deploy using Ansible') {
            steps {
                ...
                withCredentials([sshUserPrivateKey(credentialsId: '90e346b5-36e9-4e3f-b300-8d1bfbfa6b2d', keyFileVariable: 'SSH_PRIVATE_KEY')]) {
                    sh '''
                    ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i "$SSH_PRIVATE_KEY" ec2-user@ec2-44-201-198-53.compute-1.amazonaws.com 'ansible-playbook -i /etc/ansible/hosts /home/ec2-user/ansible-data/ansible-playbooks/deploy-web-app.yml'
                    '''
                }
            }
        }
